...
```{r, echo=FALSE}
# First train the full random forest model
set.seed(621)
...
# Then create the reduced version
importance_df <- as.data.frame(importance(final_rfmodel))
importance_df$Variable <- rownames(importance_df)
selected_vars <- importance_df$Variable[importance_df$MeanDecreaseAccuracy > 10]

formula_reduced <- as.formula(paste("target ~", paste(selected_vars, collapse = " + ")))

final_rfmodel_reduced <- randomForest(
  formula_reduced,
  data = clean_dataset_of_diabetes_training,
  ntree = 500,
  importance = TRUE
...
formula_reduced
final_rfmodel_reduced

# Variable Importance Plot
varImpPlot(final_rfmodel_reduced, main = "Predictor Importance")
```
...
```{r, echo=FALSE}
# Predictions
pred_class_reduced <- predict(final_rfmodel_reduced, dataset_of_diabetes_evaluation)
pred_prob_reduced <- predict(final_rfmodel_reduced, dataset_of_diabetes_evaluation, type = "prob")

# Confusion Matrix
conf_matrix_reduced <- confusionMatrix(pred_class_reduced, dataset_of_diabetes_evaluation$target)
print(conf_matrix_reduced)

# Step 5: ROC Curves and AUC for reduced model
par(mfrow = c(1, 3))
for (class in levels(dataset_of_diabetes_evaluation$target)) {
  roc_obj <- roc(
    response = as.numeric(dataset_of_diabetes_evaluation$target == class),
    predictor = pred_prob_reduced[, class]
  )
  plot(roc_obj, main = paste("Reduced ROC -", class))
  auc_val <- auc(roc_obj)
  legend("bottomright", legend = paste("AUC =", round(auc_val, 2)))
}
# Predict with reduced model
prediction_prob_reduced <- predict(final_rfmodel_reduced, clean_dataset_of_diabetes_evaluation, type = "prob")
prediction_class_reduced <- predict(final_rfmodel_reduced, clean_dataset_of_diabetes_evaluation, type = "class")

# Confusion Matrix
conf_matrix_reduced <- confusionMatrix(
  factor(prediction_class_reduced, levels = c("N", "P", "Y")),
...
# Accuracy
accuracy_reduced <- mean(prediction_class_reduced == clean_dataset_of_diabetes_evaluation$target)
...
for (class in classes_eval) {
  binary_response_reduced <- ifelse(true_labels_eval == class, 1, 0)
  pred_probs_reduced <- prediction_prob_reduced[, class]
  
  roc_curve_reduced <- roc(binary_response_reduced, pred_probs_reduced)
  
  plot(roc_curve_reduced,
       main = paste("ROC (Reduced) -", class, "\nAUC:", round(auc(roc_curve_reduced), 2)),
       col = "darkgreen", lwd = 2)
  
   cat("AUC (Reduced) for", class, ":", auc(roc_curve_reduced), "\n")
}
```
